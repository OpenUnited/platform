...
class ProductTreeGenerationService:
    def generate_initial_tree(self, product: Product, additional_context: str = "") -> Tuple[bool, str, Optional[str]]:
        try:
            success, tree, error = LLMService.generate_product_tree(
    ...
    def refine_tree(self, product: Product, current_tree: str, feedback: str) -> Tuple[bool, str, Optional[str]]:
        try:
            success, refined_tree, error = LLMService.refine_product_tree(
                current_tree=current_tree,
                feedback=feedback,
                product_name=product.name
            )
            if success:
                return True, refined_tree, None
            return False, None, error
        except Exception as e:
            return False, None, str(e)
    ...
    def save_tree(self, product: Product, tree: str) -> Tuple[bool, Optional[str]]:
        try:
            # Parse tree and create product areas
            # This would need to be implemented based on your tree format
            # and product area model structure
            return True, None
        except Exception as e:
            return False, str(e)
    ...
    def get_tree(self, product: Product) -> Optional[str]:
        ...
        """
        try:
            # Implement the actual retrieval from your database
